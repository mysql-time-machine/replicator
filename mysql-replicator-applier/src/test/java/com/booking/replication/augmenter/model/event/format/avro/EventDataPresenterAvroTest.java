...
public class EventDataPresenterAvroTest {
    ...
    public void testDefaultValuesInGeneratedAvroSchema() throws Exception {
        ...
        columns.add(new ColumnSchema("col1", DataType.INT, "int(11)", true, false));

        columns.add(new ColumnSchema("col2", DataType.INT, "int(11)", true, false)
                .setDefaultValue("10"));

        columns.add(new ColumnSchema("col3", DataType.ENUM,
                "enum('boolean','integer','string','date','datetime','boolarray','intarray','stringarray','datearray','enum')", true, false)
