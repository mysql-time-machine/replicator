    ...
    public static Map<String, Map<String, String>> stringifyRowCellsValues(
        ...
        if (columns != null) {
            ...
            if (eventType.equals("INSERT")) {
                ...
                for (int columnIndex = 0, rowIndex = 0; columnIndex < columns.size() && rowIndex < rowByteSlicesForUpdateAfter.length; columnIndex++) {
                    ...
                    if (includedColumns.get(columnIndex)) {
                        ...
                        Serializable cellValueBefore = rowByteSlicesForUpdateBefore[rowIndex++];
                        Serializable cellValueAfter = rowByteSlicesForUpdateAfter[rowIndex++];
    ...
    public static String stringifyCellValue(Map<String, String[]> cache, String columnType, Serializable cellValue, String collation) {
        ...
        if(columnType.contains("unsigned") && cellValue != null ){
            if(columnType.contains("tiny")){
                stringifiedCellValue = String.valueOf(Byte.toUnsignedInt(((Long)cellValue).byteValue()));
            }else if(columnType.contains("small")){
                stringifiedCellValue = String.valueOf(((Long)cellValue) & 0xffff);
            }else if(columnType.contains("medium")){
                stringifiedCellValue = String.valueOf(((Long)cellValue) & 0xffffff);
