...
public class SlaveFailoverIntegrationTest {
    private static final Logger LOG = LogManager.getLogger(SlaveFailoverIntegrationTest.class);
    ...
    private static MySQLContainer mysqlMasterContainer;
    private static MySQLContainer mysqlSlave1Container;
    private static MySQLContainer mysqlSlave2Container;
    ...
    public static void before() throws InterruptedException {
        ...
        LOG.info("Initializing mysql replication chain with 1 master and 2 slaves");
        ...
        mySQLMasterConfiguration = new MySQLConfiguration(
            ...
            MYSQL_PASSWORD,
            MYSQL_CONF_FILE,
            Collections.singletonList(MYSQL_MASTER_INIT_SCRIPT),
            network,
            "mysql-chain"
        );

        MySQLConfiguration mySQLSlave1Configuration = new MySQLConfiguration(
            MYSQL_SCHEMA,
            null,
            MYSQL_PASSWORD,
            MYSQL_SLAVE_CONF_FILE,
            Collections.emptyList(),
            network,
            "mysql-chain"
        );

        MySQLConfiguration mySQLSlave2Configuration = new MySQLConfiguration(
            MYSQL_SCHEMA,
            null,
            MYSQL_PASSWORD,
            MYSQL_SLAVE_CONF_FILE,
        ...
        mysqlMasterContainer = MySQLContainerProvider.startWithMySQLConfiguration(mySQLMasterConfiguration);
        mysqlSlave1Container = MySQLContainerProvider.startWithMySQLConfiguration(mySQLSlave1Configuration);
        mysqlSlave2Container = MySQLContainerProvider.startWithMySQLConfiguration(mySQLSlave2Configuration);
        mysqlActiveSchemaContainer = MySQLContainerProvider.startWithMySQLConfiguration(mySQLActiveSchemaConfiguration);

        boolean s1Initialized = initializeSlave(mysqlMasterContainer, mysqlSlave1Container, mySQLSlave1Configuration);
        boolean s2Initialized = initializeSlave(mysqlMasterContainer, mysqlSlave2Container, mySQLSlave2Configuration);
        assumeTrue("Mysql slaves failed to initialize", s1Initialized && s2Initialized);

        assumeTrue("File checkpoint failed to initialize",
            initializeGtidCheckpoint(mysqlSlave1Container, mySQLSlave1Configuration));
    ...
    private static boolean initializeSlave(MySQLContainer mysqlMasterContainer,
                                           MySQLContainer mysqlSlaveContainer,
                                           MySQLConfiguration configuration) {
        return MYSQL_RUNNER.runMysqlScript(
            mysqlSlaveContainer,
            configuration,
            new File("src/test/resources/" + MYSQL_SLAVE_INIT_SCRIPT).getAbsolutePath(),
            ImmutableMap.of("MASTER_HOSTNAME", mysqlMasterContainer.getContainerInfo().getConfig().getHostName(),
    ...
    private static boolean initializeGtidCheckpoint(MySQLContainer mysqlContainer, MySQLConfiguration configuration) {
        List<HashMap<String, Object>> rs = MYSQL_RUNNER.runMysqlQuery(mysqlContainer,
            configuration,
    ...
    public void testReplicator() {
        ...
        Replicator replicator = new Replicator(this.getConfiguration(mysqlSlave1Container));
        replicator.start();

        // Run some initial queries on master
        boolean execBinLog = MYSQL_RUNNER.runMysqlScript(
            mysqlMasterContainer,
            mySQLMasterConfiguration,
            new File("src/test/resources/" + MYSQL_TEST_SCRIPT).getAbsolutePath(),
        ...
        boolean execBinLog2 = MYSQL_RUNNER.runMysqlScript(
            mysqlMasterContainer,
            mySQLMasterConfiguration,
            new File("src/test/resources/" + MYSQL_SLAVE2_TEST_SCRIPT).getAbsolutePath(),
            Collections.emptyMap(),
            false
        );
        ...
        replicator = new Replicator(this.getConfiguration(mysqlSlave2Container));
        replicator.start();
    ...
    private Map<String, Object> getConfiguration(MySQLContainer mysqlContainer) {
        Map<String, Object> configuration = new HashMap<>();

        configuration.put(WebServer.Configuration.TYPE, WebServer.ServerType.JETTY.name());

        configuration.put(BinaryLogSupplier.Configuration.MYSQL_HOSTNAME, Collections.singletonList(mysqlContainer.getHost()));
        configuration.put(BinaryLogSupplier.Configuration.MYSQL_PORT, String.valueOf(mysqlContainer.getPort()));
    ...
    public static void after() {
        ...
        mysqlSlave1Container.stop();
        mysqlSlave2Container.stop();
